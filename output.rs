fn main() {
    ...
    while REPLAY == true {
        ...
        std::io::stdin()._line(&mut PLAYER_ONE_GESTURE).unwrap();
        PLAYER_ONE_GESTURE = PLAYER_ONE_GESTURE.trim().to_string();
        println!("Choose a gesture (rock/paper/scissors) for the following player: ");
        println!(PLAYER_TWO);
        let mut PLAYER_TWO_GESTURE = String::new();
        std::io::stdin()._line(&mut PLAYER_TWO_GESTURE).unwrap();
        ...
        std::io::stdin()._line(&mut INPUT).unwrap();
