...
const rl_PLAYER_ONE = readline.createInterface({ input, output });
                        PLAYER_ONE = await rl_PLAYER_ONE.question('');
                        rl_PLAYER_ONE.close();
console.log("Insert name of player 2:");
const rl_PLAYER_TWO = readline.createInterface({ input, output });
...
console.log("Choose a gesture (rock/paper/scissors) for the following player: ");
console.log(PLAYER_ONE);
const rl_PLAYER_ONE_GESTURE = readline.createInterface({ input, output });
                        PLAYER_ONE_GESTURE = await rl_PLAYER_ONE_GESTURE.question('');
                        rl_PLAYER_ONE_GESTURE.close();
console.log("Choose a gesture (rock/paper/scissors) for the following player: ");
console.log(PLAYER_TWO);
